    ...
    var Mark = function () {
        ...
        _createClass(Mark, [{
            ...
            value: function createSynonymsRegExp(str) {
                ...
                for (var index in syn) {
                    if (syn.hasOwnProperty(index)) {
                        ...
                        str = str.replace(new RegExp("(" + k1 + "|" + k2 + ")", "gm"), "(" + k1 + "|" + k2 + ")");
                    }
                }
                return str;
            }
            ...
            value: function createDiacriticsRegExp(str) {
                var dct = ["aÀÁÂÃÄÅàáâãäå????", "cÇç????", "d????", "eÈÉÊËèéêë??????", "iÌÍÎÏìíîï??", "l??", "nÑñ????", "oÒÓÔÕÕÖØòóôõöø??", "r??", "sŠš??", "t??", "uÙÚÛÜùúûü????", "yŸÿýÝ", "zŽž????"];
                var handled = [];
                str.split("").forEach(function (ch) {
                    dct.every(function (dct) {
                        if (dct.indexOf(ch) !== -1) {
                            ...
                            str = str.replace(new RegExp("[" + dct + "]", "gm"), "[" + dct + "]");
                            handled.push(dct);
                        }
                        return true;
                    });
            ...
            value: function mark(sv, opt) {
                ...
                kwArr.forEach(function (kw) {
                    var regex = new RegExp(_this7.createRegExp(kw), "gm"),
                        matches = 0;
                    var eachCb = function eachCb(element) {
                        matches++;
                        totalMatches++;
